...
def one_training_step(model, images_batch, labels_batch):
    with tf.GradientTape() as tape:
        ...
        per_sample_losses = tf.keras.losses.sparse_categorical_crossentropy(y_true=labels_batch, y_pred=predictions)
        average_loss = tf.reduce_mean(per_sample_losses)
    gradients = tape.gradient(target=average_loss, sources=model.weights)
